 ...
 */
public class PlaybackOverlayCustomFragment
        ...
        implements LoaderManager.LoaderCallbacks<Cursor>, TextureView.SurfaceTextureListener, VideoPlayer.Listener
{
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        ...
        switch (id) {
            case RECOMMENDED_VIDEOS_LOADER: {
                ...
                String query = args.getString(VideoContract.VideoEntry.COLUMN_DESC,"");
                Loader<Cursor> c;
                if(query.contains(" ")) {
                    c = new CursorLoader(
                            getActivity(),
    ...
    private Row createRows(int i) {
        String json = Utils.inputStreamToString(getResources().openRawResource(R.raw.cards_example));
        CardRow[] rows = new Gson().fromJson(json, CardRow[].class);
        //for (CardRow row : rows) {
        //}
        return createCardRow(rows[rows.length-(i+1)]);
    }
    ...
    private void loadQuery(String query) {
        ...
        getLoaderManager().initLoader(mSpecificVideoLoaderId++, null, this);
